...
def taylor_polynomial(x, n):
    result = 0.0
    for k in range(n + 1):
        result += derivatives_at_0[k] * (x**k) / math.factorial(k)
    return result
...
plt.plot(x_vals, y_f, label='$f(x) = e^x \cos(x)$', linewidth=2, color='black')

colors = ['red', 'blue', 'green', 'orange', 'purple', 'brown']
for i, degree in enumerate(degrees):
    y_taylor = np.array([taylor_polynomial(x, degree) for x in x_vals])
    plt.plot(x_vals, y_taylor, linestyle='--', 
             label=f'$P_{degree}(x)$', color=colors[i])
...
plt.legend()
plt.grid(True)
plt.ylim(-2, 3)  # Ajustar límites del eje y si es necesario
...
plt.figure(figsize=(8,5))
plt.plot(xnew,  poliLagrange(xnew), label="Lagrange", color="darkgreen")
plt.plot(xnew,  poli_spnat(xnew), label="Spline natural", color="purple")
plt.plot(x, y,'o', label="Nodos")
...
plt.figure(figsize=(8,5))
plt.plot(xnew,  poliLagrange(xnew), label="Lagrange", color="black")
plt.plot(xnew,  poli_spnat(xnew), label="Spline natural", color="blue")
...
x_values = np.linspace(-1, 12, 400)
plt.figure(figsize=(10,7))

plt.plot(x_values,  poli_spnat(x_values), label="Spline Natural", color="darkgreen")
...
plt.figure(figsize=(10, 6))
plt.plot(x_plot, f(x_plot), "--", color= "darkgreen")
...
f_der = lambda x : (np.cos(np.log(x)))*(1/x)
