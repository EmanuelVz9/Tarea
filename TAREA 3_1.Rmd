...
inegi_ie <- read_xls("./INEGI.xls", range = cell_rows(2:386))
```
...
```{r}
Ingresos <- names(inegi_ie)[2:6]
Ingresos
Inversion <- names(inegi_ie)[7:8]
Inversion
Precios <- names(inegi_ie)[9:11]
Precios
Exportaciones <- names(inegi_ie)[12:15]
Exportaciones
Confianza_empresarial <- names(inegi_ie)[16:21]
Confianza_empresarial
Empleo_EUA <- names(inegi_ie)[22:28]
Empleo_EUA
SP500 <- names(inegi_ie)[29:34]
SP500
Confianza_Consumidor_EUA <- names(inegi_ie)[35:43]
...
```{r}
inegi_subconjunto <- inegi_ie |> dplyr::select(Periodo, all_of(c(Ingresos, Confianza_empresarial, Empleo_EUA, SP500)))
...
#Correlacion
```{r}
data_reg <- inegi_ie %>%  select_if(is.numeric)
...
```{r, echo=FALSE}
corr_matrix <- cor(data_reg, use = "complete.obs")
...
```{r}
modelo_Industrial <- lm(Actividad_Industrial ~ Inversion_Fija_Bruta, data = inegi_ie)
summary(modelo_Industrial) 
```

```{r echo=FALSE, warning=FALSE, message=FALSE}
ggplot(inegi_ie, aes(x = Inversion_Fija_Bruta, y = Actividad_Industrial)) +
...
```{r}
modelo_Consumidor <- lm(Confianza_Consumidor ~ Confianza_Empresarial, data = inegi_ie)
summary(modelo_Consumidor) 
```

```{r echo=FALSE, warning=FALSE, message=FALSE}
ggplot(inegi_ie, aes(x = Confianza_Empresarial, y = Confianza_Consumidor)) +
  geom_point(color = "blue") +
  geom_smooth(method = "lm", se = TRUE, color = "darkgreen") +
...
# SP500 vs Dow_Jones
```{r}
modelo_SP500 <- lm(SP500 ~ Dow_Jones, data = inegi_ie)
summary(modelo_SP500) 
```

```{r echo=FALSE, warning=FALSE, message=FALSE}
ggplot(inegi_ie, aes(x = Dow_Jones, y = SP500)) +
  geom_point(color = "blue") +
  geom_smooth(method = "lm", se = TRUE, color = "darkgreen") +
...
```{r}
modelo_EUA <- lm(Empleo_EUA ~ Produccion_Manufacturera_EUA, data = inegi_ie)
summary(modelo_EUA) 
```

```{r echo=FALSE, warning=FALSE, message=FALSE}
ggplot(inegi_ie, aes(x = Produccion_Manufacturera_EUA, y = Empleo_EUA)) +
  geom_point(color = "blue") +
  geom_smooth(method = "lm", se = TRUE, color = "darkgreen") +
...
```{r}
modelo_Totales <- lm(Exportaciones_Totales ~ Exportaciones_Petroleras, data = inegi_ie)
summary(modelo_Totales) 
```

```{r echo=FALSE, warning=FALSE, message=FALSE}
ggplot(inegi_ie, aes(x = Exportaciones_Petroleras, y = Exportaciones_Totales)) +
  geom_point(color = "blue") +
  geom_smooth(method = "lm", se = TRUE, color = "darkgreen") +
...
```{r}
modelo_BMV <- lm(IPC_BMV ~ Cetes_28_Dias, data = inegi_ie)
summary(modelo_BMV) 
```

```{r echo=FALSE, warning=FALSE, message=FALSE}
ggplot(inegi_ie, aes(x = Cetes_28_Dias, y = IPC_BMV)) +
  geom_point(color = "blue") +
  geom_smooth(method = "lm", se = TRUE, color = "darkgreen") +
...
```{r}
modelo_Industrial <- lm(Actividad_Industrial ~ Inversion_Fija_Bruta + Actividades_Terciarias, data = inegi_ie)
summary(modelo_Industrial)
```
...
```{r}
modelo_Consumidor <- lm(Confianza_Consumidor ~ Confianza_Empresarial + Ingresos_Retail, data = inegi_ie)
summary(modelo_Consumidor)
```

```{r}
scatter3d(Confianza_Consumidor ~ Confianza_Empresarial + Ingresos_Retail, data = inegi_ie, 
          fit = "linear", residuals = TRUE, bg = "black", axis.scales = TRUE, 
          grid = TRUE, ellipsoid = FALSE)
```
...
```{r}
modelo_Empleo_EUA <- lm(Empleo_EUA ~ Produccion_Manufacturera_EUA + Asegurados_IMSS, data = inegi_ie)
summary(modelo_Empleo_EUA)
```

```{r}
scatter3d(Empleo_EUA ~ Produccion_Manufacturera_EUA + Asegurados_IMSS, data = inegi_ie, 
          fit = "linear", residuals = TRUE, bg = "black", axis.scales = TRUE, 
          grid = TRUE, ellipsoid = FALSE)
```
...
```{r}
modelo_Exportaciones_Totales <- lm(Exportaciones_Totales ~ Exportaciones_Petroleras + Reservas_Internacionales, data = inegi_ie)
summary(modelo_Exportaciones_Totales)
```

```{r}
scatter3d(Exportaciones_Totales ~ Exportaciones_Petroleras + Reservas_Internacionales, data = inegi_ie, 
          fit = "linear", residuals = TRUE, bg = "black", axis.scales = TRUE, 
          grid = TRUE, ellipsoid = FALSE)
```
...
```{r}
modelo_BMV <- lm(IPC_BMV ~ Cetes_28_Dias + TIIE, data = inegi_ie)
summary(modelo_BMV)
```

```{r}
scatter3d(IPC_BMV ~ Cetes_28_Dias + TIIE, data = inegi_ie, 
          fit = "linear", residuals = TRUE, bg = "black", axis.scales = TRUE, 
          grid = TRUE, ellipsoid = FALSE)
```
